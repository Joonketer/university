...
public class Professor extends JFrame implements ActionListener {
	...
	String realMajor = null;
	String cate = null;
	String deptcate = null;
	int srow = -1;
	String deptname=null, deptcode=null;
	
	String[] scombo = {"교수코드", "교수명", "학과명", "성별"};
	String[] sfield = {"code", "name", "dept_code", "gender"};


	Professor() {


		setTitle("교수관리: "+Login.username);
		setSize(700, 650);
		setLayout(null);
		...
		label9.setBounds(380, 80, 50, 20);
		label10 = new JLabel("임용년도");
		label10.setBounds(30, 110, 50, 20);
		label11 = new JLabel("학위");
		label11.setBounds(220, 110, 50, 20);
		label12 = new JLabel("학과");
		label12.setBounds(380, 110, 50, 20);
		label13 = new JLabel("연구실");
		label13.setBounds(30, 140, 50, 20);
		label14 = new JLabel("성별");
		label14.setBounds(220, 140, 50, 20);
		...
		text12.setBounds(90, 140, 100, 20);

		box1 = new JComboBox();
		box1.setBounds(440, 110, 220, 20);
		box1.addItem("학과선택");
		Major();
		radio1 = new JRadioButton("남자");
		radio1.setBounds(255, 140, 50, 20);
		radio1.setSelected(true);
		radio2 = new JRadioButton("여자");
		radio2.setBounds(310, 140, 50, 20);
		...
		box2 = new JComboBox(scombo);
		box2.setBounds(30, 25, 135, 20);
		box2.addItem("카테고리선택");
		...
		box2.addItemListener(new ItemListener() {
			public void itemStateChanged(ItemEvent ie) {
				cate = (String) box2.getSelectedItem();
				text13.requestFocus();
			}
	...
	void ProTables() {
		String[] head = { "교수코드", "이름", "주소", "주민등록번호", "휴대폰", "전화", "임용년도", "학위", "학과", "연구실", "성별" };
		String[][] row = {};
	...
	public void getListAll() {
		...
		try {
			...
			while (rs.next()) {
				...
				Object[] rowData = { code, name, addr, juminno, mphone, phone, ent_year, degree, dept, roomno,
						gender };
				dtm.addRow(rowData);
	...
	public int Regist(String code) {
		...
		String sql = "SELECT * FROM professor where code='" + text1.getText() + "'";
		try {
			pstmt = connect.conn.prepareStatement(sql);
			rs = pstmt.executeQuery();
			while (rs.next()) {
			...
			sql = "INSERT INTO professor(code,name,addr,juminno,mphone,phone,ent_year,degree,dept_code,roomno,gender) VALUES(?,?,?,?,?,?,?,?,?,?,?)";
			pstmt = connect.conn.prepareStatement(sql);
			...
			pstmt.setString(10, text12.getText());
	...
	public int Edit(String code) {
		...
		try {
			pstmt = connect.conn.prepareStatement(sql);
			pstmt.setString(11, text1.getText());
			...
			pstmt.setString(8, deptcode);	//학과코드
			pstmt.setString(9, text12.getText());
			pstmt.setString(10, realGender);
	...
	public void Search(String s) {
		s = sfield[box2.getSelectedIndex()];
		dtm.setRowCount(0);
		...
		try {
			...
			if(s.equals("dept_code")) {
				...
				sql = "SELECT * FROM professor where " + s + "='" + deptcode1 + "'";
				pstmt = connect.conn.prepareStatement(sql);
				
			}
			else {
				String sql = "SELECT * FROM professor where "+s + "='" + text13.getText() + "'";
				pstmt = connect.conn.prepareStatement(sql); 
			}
			rs = pstmt.executeQuery();
			...
			while (rs.next()) {
				...
				Object[] rowData = { code, name, addr, juminno, mphone, phone, ent_year, degree, deptname1, roomno,
						gender };
				dtm.addRow(rowData);
	...
	public void actionPerformed(ActionEvent e) {
		...
		String code, name, addr, juminno, mphone, phone, ent_year, degree, dept_code, roomno, gender;
		code = text1.getText();
		name = text2.getText();
		addr = text5.getText();
		juminno = text3.getText() + '-' + text4.getText();
		mphone = text6.getText() + '-' + text7.getText() + '-' + text8.getText();
		phone = text9.getText();
		ent_year = text10.getText();
		degree = text11.getText();
		dept_code = (String) box1.getSelectedItem();
		
		roomno = text12.getText();
		gender = realGender;
		...
		if (e.getSource() == button3) {
			...
			if (text1.getText().isEmpty()) {
				...
				JOptionPane.showMessageDialog(null, "임용년도 입력하세요");
				return;
	...
	class MyAdapter extends MouseAdapter {
		public void mouseClicked(MouseEvent e) {
			...
			String ent_year = (String) table.getValueAt(srow, 6);
			String degree = (String) table.getValueAt(srow, 7);
			String dept_code = (String) table.getValueAt(srow, 8);
			String roomno = (String) table.getValueAt(srow, 9);
			...
			text9.setText(phone);
			text10.setText(ent_year);
			text11.setText(degree);
			text12.setText(roomno);
			
			box1.setSelectedItem(dept_code);
